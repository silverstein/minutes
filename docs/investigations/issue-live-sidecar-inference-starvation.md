...
- **Duration-helper duplication**: the helpers are still separate, but both now use bounded WAV parsing. Compressed folder-watch input requires ffmpeg and fails closed with setup guidance when it is missing; with ffmpeg installed, a bounded duration probe may fall back to the configured content type only when that decoder rejects the probe.
