...
export function scoreMeridianResponses(responses, { turn1Mode = "background" } = {}) {
  ...
  const directionalCustomerObligation = [
    ...
    new RegExp(
      String.raw`\b${vendorActor}\b[^;.!?]{0,35}\bowes?\s+(?:the\s+)?${customerActor}\b[^;.!?]{0,30}(?:\$\s*200|two\s+hundred\s+dollars?)(?:\s+(?:service\s+)?credits?)?\s+for\s+${simpleScopedAutomatedError}`,
    ),
    new RegExp(
  ...
  const hasConfidenceGatedHumanFallback = [
    ...
    /\b(?:rout(?:e|ing)|send|return|revert|switch|leave|keep|retain)\w*\b[^;.!?]{0,55}\b(?:uncertain|low[- ]confidence|below[- ]threshold|below\s+(?:the\s+)?(?:confidence\s+)?threshold|high[- ]risk|remaining|remainder)\b[^;.!?]{0,45}\bhuman(?:s| handling| review|[- ]in[- ]the[- ]loop)?\b/,
    /\b(?:uncertain|low[- ]confidence|below[- ]threshold|below\s+(?:the\s+)?(?:confidence\s+)?threshold|high[- ]risk|remaining|remainder)\b[^;.!?]{0,45}\b(?:rout(?:e|ing)|send|return|revert|switch|leave|keep|retain)\w*\b[^;.!?]{0,45}\bhuman(?:s| handling| review|[- ]in[- ]the[- ]loop)?\b/,
  ...
  const negatesHumanFallback =
    /\b(?:do\s+not|don't|never)\b[^;.!?]{0,45}\b(?:route|send|return|leave|keep|retain)\b[^;.!?]{0,55}\b(?:uncertain|low[- ]confidence|below[- ]threshold|remainder|rest|tickets?|work)\b[^;.!?]{0,45}\b(?:humans?|human\s+(?:review|handling)|support\s+team|people)\b|\b(?:route|send|keep|leave)\w*\b[^;.!?]{0,45}\b(?:uncertain|low[- ]confidence|below[- ]threshold|remainder|rest|tickets?|work)\b[^;.!?]{0,35}\b(?:away\s+from|out\s+of)\b[^;.!?]{0,20}\bhuman\s+(?:review|handling)\b|\bhuman\s+(?:review|handling)\b[^;.!?]{0,35}\b(?:should|must|will|can)?\s*exclude\w*\b[^;.!?]{0,35}\b(?:uncertain|low[- ]confidence|below[- ]threshold|below\s+(?:the\s+)?(?:confidence\s+)?threshold)\b|\b(?:route|send|keep|leave)\w*\b[^;.!?]{0,45}\b(?:uncertain|low[- ]confidence|below[- ]threshold|remainder|rest)\b[^;.!?]{0,35}\b(?:human[- ]free|automation[- ]only|fully\s+automated)\b|\b(?:ignore|discard|remove|bypass)\b[^;.!?]{0,30}\b(?:that|the)\b[^;.!?]{0,15}\b(?:gate|threshold|fallback)\b[^;.!?]{0,35}\bautomat\w*\b[^;.!?]{0,20}\b(?:remainder|rest|all|too)\b|\bautomat\w*\b[^;.!?]{0,35}\b(?:uncertain|low[- ]confidence|below[- ]threshold|remainder|rest)\b[^;.!?]{0,20}\b(?:too|anyway|also)\b/.test(
      turn1,
    );
