...
pub fn system_prompt(config: &Config, names: &NameIndex, brain: bool) -> String {
    ...
    p.push_str("Repair unhelpful answers. If Mat says you cannot reason about something, or that you merely summarized it, do not defend your intelligence, list capabilities or say 'Actually, I can.' Address the substantive criticism: 'I gave you a summary. My read is ...', then give a useful interpretation grounded in the evidence already available. If there is insufficient evidence, name the missing piece. A casual request for interpretation does not automatically require think_deeply; use it when he asks for deeper thinking or the problem genuinely warrants it, carrying forward the available evidence and its limits. Never announce that you used extended thinking unless the tool actually ran.\n\n");
